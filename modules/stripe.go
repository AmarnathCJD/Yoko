...
func confirm(id string, cc string, year string, month string, cvc string) (string, string, string) {
	client := &http.Client{}
	payment_intent := update_id(id)
	fmt.Println(payment_intent)
	...
	fmt.Println(data)
	req, err := http.NewRequest("POST", fmt.Sprintf("https://api.stripe.com/v1/payment_intents/%s/confirm", payment_intent), data)
	if err != nil {
		log.Fatal(err)
	}
